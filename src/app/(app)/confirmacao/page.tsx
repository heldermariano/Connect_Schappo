...
function StatusBadge({ status, indStatus }: { status: string | null; indStatus: string | null }) {
  ...
  if (indStatus === 'C') {
    return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-green-500/20 text-green-400">Confirmado (ERP)</span>;
  }
  if (indStatus === 'F') {
    return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-red-500/20 text-red-400">Faltou</span>;
  }
  if (indStatus === 'M') {
    return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-500/20 text-blue-400">Marcado</span>;
  }

  // Status de confirmacao local (WhatsApp)
  if (!status) {
    return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-500/20 text-gray-400">Pendente</span>;
  }
  if (status === 'enviado') {
    return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-500/20 text-yellow-400">Enviado</span>;
  }
  if (status === 'confirmado') {
    return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-green-500/20 text-green-400">Confirmado</span>;
  }
  if (status === 'desmarcou') {
    return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-red-500/20 text-red-400">Desmarcou</span>;
  }
  if (status === 'sem_resposta') {
    return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-orange-500/20 text-orange-400">Sem resposta</span>;
  }
  if (status === 'reagendar') {
    return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-purple-500/20 text-purple-400">Quer reagendar</span>;
  }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950 text-white">
      ...
      <div className="px-6 py-4 border-b border-gray-800">
        ...
        <p className="text-sm text-gray-400 mt-1">Confirme agendamentos via WhatsApp</p>
      </div>
      ...
      {resultado && (
        <div className="mx-6 mt-3 px-4 py-2 bg-green-500/10 border border-green-500/30 rounded-lg text-green-400 text-sm flex gap-4">
          <span>{resultado.enviados} enviado(s)</span>
          {resultado.falhas > 0 && <span className="text-red-400">{resultado.falhas} falha(s)</span>}
          {resultado.sem_telefone > 0 && <span className="text-yellow-400">{resultado.sem_telefone} sem telefone</span>}
        </div>
      )}

      {/* Resumo */}
      {agendamentos.length > 0 && (
        <div className="px-6 py-3 border-b border-gray-800 flex flex-wrap gap-4 text-sm">
          <span className="text-gray-400">{resumo.total} agendado(s)</span>
          <span className="text-green-400">{resumo.confirmados} confirmado(s)</span>
          <span className="text-yellow-400">{resumo.enviados} enviado(s)</span>
          <span className="text-gray-400">{resumo.pendentes} pendente(s)</span>
          {resumo.desmarcou > 0 && <span className="text-red-400">{resumo.desmarcou} desmarcou</span>}
          {resumo.reagendar > 0 && <span className="text-purple-400">{resumo.reagendar} reagendar</span>}
          {resumo.semTel > 0 && <span className="text-orange-400">{resumo.semTel} sem telefone</span>}
        </div>
      )}
              ...
              <tr className="text-gray-400 text-xs uppercase">
                <th className="px-4 py-3 text-left w-10">
                  <input
                    ...
                    className="rounded border-gray-600 bg-gray-800 text-schappo-500 focus:ring-schappo-500"
                  />
                </th>
            ...
            <tbody className="divide-y divide-gray-800">
              {agendamentos.map((ag: AgendamentoPaciente) => {
                ...
                return (
                  <tr
                    key={ag.chave}
                    className={`hover:bg-gray-800/50 ${isSelected ? 'bg-schappo-500/5' : ''}`}
                  >
                    ...
                    <td className="px-4 py-3 font-mono text-gray-300">{ag.des_hora || '--'}</td>
                    <td className="px-4 py-3">
                      <div className="font-medium text-white">{ag.nom_paciente_completo || 'Sem nome'}</div>
                      {ag.nom_resp && <div className="text-xs text-gray-500">Resp: {ag.nom_resp}</div>}
                    </td>
                    <td className="px-4 py-3 text-gray-400">{ag.convenio || '--'}</td>
                    <td className="px-4 py-3">
                      {ag.telefone_whatsapp ? (
                        <span className="text-green-400">{formatPhone(ag.telefone_whatsapp)}</span>
                      ) : (
                        <span className="text-red-400 text-xs">Sem telefone</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-gray-400 max-w-[200px] truncate" title={ag.des_procedimento || ''}>
                      {ag.des_procedimento || '--'}
                    </td>
                    <td className="px-4 py-3">
                        ...
                        {hasConfirmacao && ag.confirmacao?.status === 'enviado' && (
                          <>
                            ...
                            <button
                              ...
                              disabled={isUpdating}
                              className="px-2 py-1 text-xs rounded bg-orange-500/20 text-orange-400 hover:bg-orange-500/30 disabled:opacity-50"
      ...
      {selecionados.size > 0 && (
        <div className="px-6 py-3 border-t border-gray-800 bg-gray-900 flex items-center justify-between">
          <span className="text-sm text-gray-400">{selecionados.size} paciente(s) selecionado(s)</span>
          <button
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-gray-900 rounded-xl border border-gray-700 shadow-2xl w-full max-w-2xl mx-4 flex flex-col max-h-[80vh]">
            ...
            <div className="px-6 py-4 flex-1 overflow-y-auto">
              ...
              <div className="mb-3">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    className="flex-1 px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white focus:outline-none focus:border-schappo-500"
                  >
                  ...
                  {templateSelecionado && !templates.find(t => t.id === templateSelecionado)?.padrao && (
                    <button
                      ...
                      className="px-2 py-1 text-xs text-red-400 hover:text-red-300 border border-red-500/30 rounded-lg"
              ...
              <textarea
                ...
                rows={12}
                className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white placeholder-gray-500 focus:outline-none focus:border-schappo-500 resize-none font-mono"
              />
              ...
              {!showSalvarTemplate ? (
                ...
                <div className="mt-2 flex gap-2 items-center">
                  <input
                    ...
                    className="flex-1 px-2 py-1 bg-gray-800 border border-gray-700 rounded text-sm text-white placeholder-gray-500 focus:outline-none focus:border-schappo-500"
                  />
                  <button
              ...
              {/* Preview */}
              <div className="mt-4">
                <label className="block text-xs text-gray-400 mb-1">Preview (primeiro paciente)</label>
                <div className="px-3 py-2 bg-gray-800/50 border border-gray-700 rounded-lg text-sm text-gray-300 whitespace-pre-wrap font-mono">
                  {(() => {
