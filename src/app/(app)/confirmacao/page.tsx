...
function StatusBadge({ status, indStatus }: { status: string | null; indStatus: string | null }) {
  ...
  if (!status) {
    return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">Pendente</span>;
  }
  if (status === 'enviado') {
  ...
  return (
    <div className="flex flex-col h-full bg-white text-gray-900">
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <h1 className="text-xl font-bold text-gray-900">Confirmacao de Agendamento</h1>
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex flex-wrap gap-4 items-end">
        ...
        <div className="relative flex-1 min-w-[250px] max-w-[400px]">
          ...
          <input
            ...
            className="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-schappo-500"
          />
            ...
            <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
              {medicosFiltrados.map(m => (
                ...
                >
                  ...
                  {m.num_crm && <span className="text-gray-400 text-xs">CRM {m.num_crm}</span>}
                </button>
              ))}
            </div>
          )}
      ...
      {/* Erro */}
      {error && (
        <div className="mx-6 mt-3 px-4 py-2 bg-red-50 border border-red-200 rounded-lg text-red-600 text-sm">
          {error}
        </div>
      )}
      ...
      {resultado && (
        <div className="mx-6 mt-3 px-4 py-2 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm flex gap-4">
          ...
          {resultado.falhas > 0 && <span className="text-red-600">{resultado.falhas} falha(s)</span>}
          {resultado.sem_telefone > 0 && <span className="text-yellow-600">{resultado.sem_telefone} sem telefone</span>}
        </div>
      )}
      ...
      {agendamentos.length > 0 && (
        <div className="px-6 py-3 border-b border-gray-200 flex flex-wrap gap-4 text-sm">
          <span className="text-gray-600">{resumo.total} agendado(s)</span>
            ...
            <tbody className="divide-y divide-gray-100">
              {agendamentos.map((ag: AgendamentoPaciente) => {
                ...
                return (
                  <tr
                    key={ag.chave}
                    className={`hover:bg-gray-50 ${isSelected ? 'bg-schappo-50' : ''}`}
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      {ag.nom_resp && <div className="text-xs text-gray-400">Resp: {ag.nom_resp}</div>}
                    </td>
                    <td className="px-4 py-3 text-gray-600">{ag.convenio || '--'}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-gray-600 max-w-[200px] truncate" title={ag.des_procedimento || ''}>
                      {ag.des_procedimento || '--'}
                    </td>
                    <td className="px-4 py-3">
      ...
      {selecionados.size > 0 && (
        <div className="px-6 py-3 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
          <span className="text-sm text-gray-600">{selecionados.size} paciente(s) selecionado(s)</span>
          <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-white rounded-xl border border-gray-200 shadow-2xl w-full max-w-2xl mx-4 flex flex-col max-h-[80vh]">
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900">Enviar Confirmacao via WhatsApp</h2>
            ...
            <div className="px-6 py-4 flex-1 overflow-y-auto">
              ...
              <textarea
                ...
                rows={12}
                className="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-schappo-500 resize-none font-mono"
              />
              ...
              {!showSalvarTemplate ? (
                ...
                <div className="mt-2 flex gap-2 items-center">
                  <input
                    ...
                    className="flex-1 px-2 py-1 bg-white border border-gray-300 rounded text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-schappo-500"
                  />
                  <button
